...
type Props = {
  ...
  // OPTIONAL: keep App.tsx in sync with the latest transcript (for saving the study bundle)
...
export default function ChatPane({ onUserPrompt, onHistoryChange }: Props) {
  ...
  function handleSend(text: string) {
    ...
    let acc = "";
    streamChat({ prompt: text, model, files }, (delta) => {
      acc += delta;
      setMessages((m) => {
  ...
  return (
    <div style={{ display: "grid", gridTemplateRows: "auto 1fr auto", gap: 12, height: "100%", minHeight: 0 }}>
      {/* Toolbar (models/files/save/load) */}
      <div className="panel">
        <div className="panel-body" style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input type="file" multiple accept=".pdf,.txt" onChange={(e) => setFiles(Array.from(e.target.files ?? []))} />
          ...
          <select onChange={(e) => handleLoad(e.target.value)} value="">
            <option value="" disabled>Load saved chat</option>
            {chats.map((c) => (
